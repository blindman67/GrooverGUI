//export GUISlider function(GUI){
    const slider = (function(){
        ...
        function createControl(name,options){
            ...
            /*Math.ceil(-Math.log(options.step))-1*///}
            function mouseSliderDrag(event){
                if(event.type === "mouseup"){
                    ...
                    if (!control.mouseOver) {elements.sliderTop.style.background = preColour}
                }
                var sliderPos = 0;
            ...
            function wheelMove(event){
                if(!event.exclusive){
                    var step = event.wheelDelta === undefined ? -event.deltaY : event.wheelDelta;
                    stepValue(step, event);
                    event.preventDefault();
                    control.dialog.focus();
                }
            ...
            const elements = {
                ...
                slideBack   : !options.showTicks ? $("$D ", uName+"SliderBack", "slideBack") :
                                $(GUI.image.getSpriteSheetImage("sliderTicks",0),{id:uName+"SliderBack",className:"slideBack"}),
            ...
            $$(elements.container,[
                ...
                //$$(elements.slideBack,[elements.sliderTop]),
                elements.slideBack,
                elements.sliderTop,
                elements.textInput,
            ]);
