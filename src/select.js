//export GUISelector function(GUI){
    ...
    const selection = (function(){
        ...
        function createControl(name,options){
            ...
            function setArrows(){
                var maxScroll = (options.items.length) * (CONTROL_HEIGHT - 2) - selectBoxLines * (CONTROL_HEIGHT - 2);
                if(scrollPos < 0){
                    elements.scrollArrowUp.style.display = "block";
                }else{
                    elements.scrollArrowUp.style.display = "none";
                }
                if(scrollPos > -maxScroll){
                    elements.scrollArrowDown.style.display = "block";
                }else{
                    elements.scrollArrowDown.style.display = "none";
                }
            }
            function scrollItems(event){
                ...
                if(event.type === "keydown"){
                    ...
                    scrollPos = scrollPos + CONTROL_HEIGHT * Math.sign(event.wheelDelta) * 2;
                    scrollPosC += 1 * Math.sign(event.wheelDelta);
                }
                scrollPos = scrollPos > 0 ? 0 : scrollPos;
                var maxScroll = (options.items.length) * (CONTROL_HEIGHT - 2) - selectBoxLines * (CONTROL_HEIGHT - 2);
                scrollPos = scrollPos < -maxScroll ? -maxScroll : scrollPos;
                if(scrollAnimId === undefined){
                    scrollAnimId = GUI.animation.addTransition(1000, {}, animations.scrolling);
                }
                setArrows();
            }
            function clicked(event){  // open select box
                if(!event.exclusive){
                    ...
                    GUI.animation.addTransition(
                        0.2, {
                            size : selectBoxLines * (CONTROL_HEIGHT-2)
                        },
            ...
            function wheelSelect(event){
                if(!event.exclusive){
                    control.dialog.focus();
                    selectNext(-Math.sign(event.wheelDelta || -event.deltaY));
                    event.preventDefault();
                }
            }
